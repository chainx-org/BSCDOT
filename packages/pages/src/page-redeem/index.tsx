...
export default function RedeemContent({ className }: Props): React.ReactElement<Props> {
  ...
  const {hasAccounts, currentAccount, RedeemRecords } = useContext(PolkadotAccountsContext);
  const redreemlen = RedeemRecords.length
  const {platonAccount, hasPlatonAccount} = useContext(PlatonAccountsContext)
  ...
  return (
    <Wrapper className={`contentWrapper ${className}`}>
      { hasPlatonAccount && hasAccounts?
        ...
        <Records className = "right" title="赎回记录" records={RedeemRecords} recordlen={redreemlen} arrows={true} />
      </Wrapper>
  );
}
